    ...
    try {
      ...
      for (var entry in _groupedResponses.entries) {
        ...
        if (nokItems.isNotEmpty) {
          print('📸 Category "${entry.key}" has ${nokItems.length} NOK items');
          for (var item in nokItems) {
            print('   - ${item['item_text'] ?? 'Unknown'} (photos: ${item['photo_urls']})');
          }
          nokItemsByCategory[entry.key] = nokItems;
        }
      }
      
      print('📊 Total NOK items across all categories: ${nokItemsByCategory.values.fold<int>(0, (sum, list) => sum + list.length)}');
                ...
                return pw.Column(
                  children: [
                    ...
                    pw.Row(
                      ...
                      children: [
                        pw.Text(
                          'Lampiran Foto - $categoryName',
                          style: pw.TextStyle(fontSize: 10, color: PdfColors.grey600),
                        ),
                        pw.Text(
                          'Halaman ${context.pageNumber} · Generated by TnD System',
                          style: pw.TextStyle(fontSize: 10, color: PdfColors.grey600),
                        ),
                      ],
                    ),
